...
const Vec2D = struct {
    ...
    fn L1(self: Vec2D) i32 {
        return std.math.abs(i32, self.x) + std.math.abs(i32, self.y);
    }
};
...
const Rope = struct {
    ...
    fn moveHead(self: *Rope, move: Vec2D) void {
        self.head = self.head.add(move);
        var diff = self.head.sub(self.tail);
        ...
        self.tail = self.tail.add(tailMove);
    }
};
...
pub fn process(allocator: std.mem.Allocator, input: []const u8) !void {
    ...
    while (lines.next()) |line| {
        ...
        for (range(n)) |_| {
            rope.moveHead(dir);
            grid[@intCast(u32, rope.tail.x + N)][@intCast(u32, rope.tail.y + N)] += 1;
        }
    }
