...
fn find(alloc: std.mem.Allocator, net: Network, init: Network.State) !u32 {
    const Finder = struct {
        ...
        const StateQueue = std.PriorityQueue(QueueElement, void, struct {
            fn inner(_: void, a: QueueElement, b: QueueElement) std.math.Order {
                return std.math.order(b.upper_bound, a.upper_bound);
            }
        }.inner);
        ...
        fn init(allocator: std.mem.Allocator, network: Network) !Self {
            ...
            return Self{
                ...
                .queue = StateQueue.init(allocator, {}),
                .pathes = pathes,
            };
        }
        ...
        fn find(self: *Self, initial_state: Network.State) !void {
            try self.queue.add(.{ .state = initial_state, .upper_bound = std.math.maxInt(u32) });

            while (self.queue.removeOrNull()) |el| {
                var state = el.state;
                if (el.upper_bound <= self.max_pressure) {
                    break;
                }
                ...
                for (self.openable_valves.items) |valve| {
                    ...
                    const distance = self.pathes.rows[valve.id][agent.valve_id];
                    ...
                    if (remaining_time < new_state.agents[1].remaining_time) {
                        std.mem.swap(@TypeOf(new_state.agents[0]), &new_state.agents[0], &new_state.agents[1]);
                    }

                    var max_remaining_time: u32 = new_state.agents[0].remaining_time;
                    var max_releasable_pressure: u32 = new_state.pressure;
                    for (self.openable_valves.items) |new_valve| {
                        if (!state.is_open(new_valve.id)) {
                            max_releasable_pressure += max_remaining_time * new_valve.flow;
                            max_remaining_time -|= 1;
                        }
                    }

                    self.max_pressure = @maximum(self.max_pressure, new_state.pressure);
                    if (self.max_pressure < max_releasable_pressure) {
                        try self.queue.add(.{ .state = new_state, .upper_bound = max_releasable_pressure });
                    }
                }
            }
