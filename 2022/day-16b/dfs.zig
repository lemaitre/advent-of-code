...
fn find(alloc: std.mem.Allocator, net: Network, init: Network.State) !u32 {
    const Finder = struct {
        ...
        fn find(self: *Self, state: Network.State) std.mem.Allocator.Error!void {
            self.max_pressure = @maximum(self.max_pressure, state.pressure);

            var max_remaining_time: u32 = state.agents[0].remaining_time;
            var max_releasable_pressure: u32 = state.pressure;
            for (self.network.valves.items) |valve| {
            ...
            for (self.network.valves.items) |valve| {
                ...
                if (new_state.agents[0].remaining_time < new_state.agents[1].remaining_time) {
                    std.mem.swap(@TypeOf(new_state.agents[0]), &new_state.agents[0], &new_state.agents[1]);
                }

                try self.find(new_state);
            }
        }
    };
