...
fn Simulator(comptime T: type) type {
    return struct {
        ...
        fn simulate(self: *Self, state: State(T)) std.mem.Allocator.Error!void {
            ...
            for ([4]ResourceType{ .ore, .clay, .obsidian, .geode }) |res_type| {
...
pub fn process(allocator: std.mem.Allocator, input: []const u8, n: u16) !void {
    ...
    while (lines.next()) |line| {
        ...
        var score = simulator.max_geode * i;
        try std.fmt.format(stdout, "Blueprint {}: {} geodes, {} points\n", .{ i, simulator.max_geode, score });
        i += 1;
