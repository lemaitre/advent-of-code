...
fn findDir(root: *const TreeNode, threshold: usize) ?*const TreeNode {
    ...
    if (root.entries) |nodes| {
        ...
        for (nodes.items) |*node| {
            var candidate_ = findDir(node, threshold);
...
pub fn process(allocator: std.mem.Allocator, input: []const u8) !void {
    ...
    var free_space = 70000000 - root.size;
    assert(free_space < 30000000);
    var threshold = 30000000 - free_space;
    try std.fmt.format(stdout, "size: {}\n", .{findDir(&root, threshold).?.size});
}
