...
pub fn process(allocator: std.mem.Allocator, input: []const u8) !void {
    ...
    while (lines.next()) |line| {
        i += 1;
        var lhs = try Element.parse(allocator, line);
        var rhs = try Element.parse(allocator, lines.next().?);
        try std.fmt.format(stdout, "{}: 1\n", .{i});
        try lhs.print(stdout, 1);
        try std.fmt.format(stdout, "{}: 2\n", .{i});
        try rhs.print(stdout, 1);
